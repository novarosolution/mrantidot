...
export function BookingStatusBanner({ status }: { status: BookingStatus }) {
  const palette = statusColors[status] ?? statusColors.pending;
  const guidance = bookingStatusMessage(status);
  ...
  return (
    <View style={[styles.wrap, { backgroundColor: palette.bg }, shadows.floating]}>
      ...
      <View style={styles.content}>
        ...
        {guidance ? <Text style={[styles.hint, { color: palette.text }]}>{guidance}</Text> : null}
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  hint: { fontFamily: fonts.body, fontSize: 12, marginTop: 4, opacity: 0.9 },
});
